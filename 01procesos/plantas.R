...
plantas <- unique(ml[,c("ID","plantas")])
head(plantas)
plantas <- plantas[order(plantas$ID),]
write.csv(plantas, file = "../02salidas/plantas.csv", row.names = FALSE)

head(plantas)
plantas$value <- 1

plantas_wider <- pivot_wider(plantas, names_from =  "ID", values_from = "value", values_fill = 0)
write.csv(plantas_wider, file = "../02salidas/plantas_wider.csv", row.names = FALSE)
